  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div>
          <div className="text-xs text-surface-400 uppercase tracking-wider mb-3">TCP/IP Model (Forouzan 5-Layer)</div>
            ...
            {[...TCPIP_LAYERS].reverse().map(l => (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-surface-400">{l.pdus}</div>
                </div>
                <div className="text-xs text-surface-500">{l.protocols.slice(0,2).join(', ')}</div>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="text-xs text-surface-400 uppercase tracking-wider mb-3">OSI ↔ TCP/IP Mapping</div>
          <div className="space-y-2">
            {MODEL_COMPARE.map((m, i) => (
              <div key={i} className="flex items-stretch gap-2">
                <div className="flex-1 rounded-xl border border-white/10 p-3" style={{ background: 'rgba(255,255,255,0.04)' }}>
                  <div className="text-xs text-surface-400 mb-1">OSI</div>
                  {m.osi.map(o => (
                ...
                <div className="flex-1 rounded-xl border border-white/10 p-3" style={{ background: m.color + '18' }}>
                  <div className="text-xs text-surface-400 mb-1">TCP/IP</div>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={d.title} className="rounded-xl border border-white/10 p-4" style={{ background: 'rgba(255,255,255,0.03)' }}>
            ...
            <div className="text-xs text-surface-400 mb-1">OSI: <span className="text-white">{d.osi}</span></div>
            <div className="text-xs text-surface-400">TCP/IP: <span className="text-white">{d.tcpip}</span></div>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="rounded-2xl border border-white/10 p-6" style={{ background: layer.bg }}>
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <div className="text-sm text-surface-400">PDU: {layer.pdus} · Maps to: {layer.osiMapping}</div>
          </div>
        </div>
        <p className="text-surface-300 text-sm leading-relaxed">{layer.role}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3 items-center">
        ...
        <button onClick={reset} className="px-4 py-2 rounded-xl border border-white/10 text-sm text-surface-400 hover:text-white transition-all">
          ↺ Reset
        </button>
        <div className="text-xs text-surface-500 ml-2">Step {Math.max(0, step + 1)} / {TCP_HANDSHAKE_STEPS.length}</div>
      </div>
          ...
          <div className="rounded-2xl border border-white/10 p-6 overflow-hidden" style={{ background: 'rgba(0,0,0,0.3)' }}>
            ...
            <div className="flex justify-between mb-4">
              <div className="flex flex-col items-center gap-1">
                ...
                <div className="text-xs text-surface-500">192.168.1.10</div>
              </div>
              <div className="flex flex-col items-center gap-1">
                ...
                <div className="text-xs text-surface-500">93.184.216.34</div>
              </div>
            </div>
          ...
          <AnimatePresence mode="wait">
            {step >= 0 ? (
              ...
              >
                ...
                <p className="text-sm text-surface-300 leading-relaxed">{TCP_HANDSHAKE_STEPS[step].desc}</p>
              </motion.div>
            ) : (
              <div className="rounded-2xl border border-white/10 p-5 text-center text-surface-500 text-sm">
      ...
      <div className="rounded-xl border border-white/10 p-5" style={{ background: 'rgba(0,0,0,0.3)' }}>
        <div className="text-xs font-semibold text-surface-400 mb-3">TCP STATE TRANSITIONS (KEY STATES)</div>
...
function DataFlowTab() {
  ...
  return (
    <div className="space-y-6">
      <div className="text-sm text-surface-400">Follow an HTTP GET request from browser to wire and back.</div>
        ...
        {FLOW_STEPS.map((s, i) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
              step === i ? 'border-white/30' : 'border-white/10 text-surface-500 hover:text-surface-300'
            }`}
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-surface-400">{current.name} Layer</div>
              </div>
            </div>
            ...
            <p className="text-sm text-surface-300 leading-relaxed">{current.desc}</p>
          </motion.div>
        </AnimatePresence>

        {/* Cumulative layers visual */}
        <div>
          <div className="text-xs text-surface-400 uppercase tracking-wider mb-3">PDU at layer {current.layer}</div>
          ...
          <div className="mt-4 rounded-xl border border-white/10 p-4" style={{ background: 'rgba(0,0,0,0.3)' }}>
            <div className="text-xs font-semibold text-surface-400 mb-2">DECAPSULATION (Receiver)</div>
      ...
      <div className="flex gap-3">
        <button onClick={() => setStep(s => Math.max(0, s - 1))} disabled={step === 0}
          className="flex-1 py-2 rounded-xl border border-white/10 text-sm text-surface-400 hover:text-white disabled:opacity-30 transition-all">
          ← Up a Layer
        </button>
  ...
  return (
    <div className="min-h-screen p-6 space-y-6" style={{ background: 'linear-gradient(135deg, #0d1117 0%, #0f1923 100%)' }}>
      ...
      <div className="rounded-3xl border border-white/10 p-8 overflow-hidden relative"
        style={{ background: 'linear-gradient(135deg, rgba(34,197,94,0.15) 0%, rgba(59,130,246,0.15) 100%)' }}>
        <div className="absolute top-0 right-0 w-80 h-80 rounded-full opacity-10"
          style={{ background: 'radial-gradient(circle, #22c55e, transparent)', transform: 'translate(25%, -25%)' }} />
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-3">
            <span className="text-4xl">🔗</span>
            <div className="px-3 py-1 rounded-full text-xs font-semibold border border-green-500/30 text-green-400 bg-green-500/10">
              Forouzan — Data Communications & Networking
            </div>
          </div>
          <h1 className="text-4xl font-black text-white mb-2">TCP/IP Architecture</h1>
          <p className="text-surface-400 max-w-2xl leading-relaxed">
            The practical protocol suite that powers the Internet. Based on Forouzan's 5-layer model —
            Physical, Data Link, Internet (IP), Transport (TCP/UDP), and Application. Covers TCP
            3-way handshake, IP addressing, and complete data flow from browser to wire.
          </p>
          <div className="flex gap-4 mt-4 text-sm flex-wrap">
            {['TCP/UDP', 'IPv4/IPv6', 'HTTP/DNS', '3-Way Handshake', 'NAT & CIDR'].map(tag => (
